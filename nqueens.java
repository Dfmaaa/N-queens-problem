...
public class nqueens {
	...
	static void diagonal(int x, int y){
		...
		int yincreaser=x+1;
		int ydecreaser=y-1;
		int xincreaserDup=x+1;
		int xdecreaserDup=x-1;
		int yincreaserDup=x+1;
		int ydecreaserDup=y-1;
		//front left
		...
		//front right
		while(xincreaserDup<8&ydecreaser>-1){
			board[xincreaserDup][ydecreaser].isAttacked=true;
			xincreaserDup++;
			ydecreaserDup--;
		}
		//back left
		while(xdecreaserDup>-1&yincreaserDup<8){
			board[xdecreaserDup][yincreaserDup].isAttacked=true;
			xdecreaserDup--;
			yincreaserDup++;
		}
	}
	...
	static void printBooleanValues(){
		for(int x=0;x<8;x++){
			for(int y=0;y<8;y++){
				System.out.print(board[x][y].isAttacked+" ");
		    }
		    System.out.println("\n");
		}
	}
